...
public class WorldGenDungeonsModded extends WorldGenerator
{
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        if (j2 >= 1 && j2 <= 5)
        {
            ...
            {
                // DEBUG
                System.out.println("Replacing dungeon mob spawner at "+tileentity.getPos());
                
                ((TileEntityMobSpawner)tileentity).getSpawnerBaseLogic().setEntityId(EntityRegistry.getEntry(EntityWither.class).getRegistryName()); 
                tileentity.getTileData().setBoolean(MainMod.MODID, true);;
            }
            else
            {
